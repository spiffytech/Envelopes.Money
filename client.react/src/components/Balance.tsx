...
export default function Balance({balance, defaultDaysToRender}: {balance: AccountBalance, defaultDaysToRender: number}) {
  ...
  const circles =
    renderableDatapoints.map((data, i) => ({
      x: xScale(i),
      y: yScale(data),
      data,
   ...
   <div
    ...
    >
      ...
      <svg viewBox="0 0 400 100">
        ...
        <path d={d!} fill="none" strokeWidth="2" className="stroke-current text-gray-500"></path>
        { circles.map((circle) => <circle key={circle.x} cx={circle.x} cy={circle.y} r="3" color="purple">
          <title>{circle.date.toLocaleDateString()} - {circle.data}</title>
        </circle>
        )}
      </svg>
    </div>
