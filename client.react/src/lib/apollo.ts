...
function mkClient(token: string, isAdmin = false) {
  const httpUri = process.env.REACT_APP_GRAPHQL_HTTP_HOST;
  if (!httpUri) throw new Error('Missing Apollo GraphQL endpoint');
  const wssUri = process.env.REACT_APP_GRAPHQL_WSS_HOST;
  ...
  const errorLink = onError(({ graphQLErrors, networkError }) => {
    if (graphQLErrors) {
      graphQLErrors.map(error =>
        console.error(`[GraphQL error]: ${JSON.stringify(error, null, 4)}`)
      );
    }
  ...
  const primaryLink = split(
    ({ query }) => {
      const thing = getMainDefinition(query);
      return (
        thing.kind === 'OperationDefinition' &&
        thing.operation === 'subscription'
      );
    },
    wssLink,
    httpLink
